...
from nmpc.diverse_functions import nominal_dynamics, cr_array_error
...
cr_e_1 = cr_array_error(x_1, e_seq, horizon, blfx, blfu)
cr_e_2 = cr_array_error(x_2, e_seq, horizon, blfx, blfu)
...
myp_1.plot_basic_multi_traj(y_scale_log=False, set_x_ticks=True)
ax[0].plot(e_seq, cr_e_1, marker='*', color='black', markersize=12)
myp_2 = MonteCarloPlotter(ax[1], e_seq, cr_table,
                          info_text2, info_font, info_color=(0.15, 0.15, 0.15), marker=True)
myp_2.plot_basic_multi_traj(y_scale_log=False, set_x_ticks=True)
ax[1].plot(e_seq, cr_e_2, marker='*', color='black', markersize=12)
plt.tight_layout()
plt.savefig("competitive_ratio_multi_traj_bound.pdf", format="pdf",
